...
from AbstractClass import graph_modeling

class modeling_biddings(graph_modeling):
    
    def __init__ (self):
        f=open("graph-modeling-module\source\config.json")
        data= json.load(f)
        #all graph info
    ...
    def define_graphs(self):
        df_node_info = pd.read_csv(self.graph_info_file_path, delimiter=';', dtype={'num_documento': object})
    ...
    def define_edges(self):
        ...
        def __determine_weight(node_1_in, node_2_in, node_1_out, node_2_out, graph_date, monthly_decay):
            ...
            if graph_date < bond_start : return 0
           
            if graph_date <= bond_end : return 1
        ...
        corporate_bonds = corporate_bonds[corporate_bonds[self.node1_enters] != self.node1_enters]

        corporate_bonds['weight'] = corporate_bonds.apply(lambda row: __determine_weight(row[self.node1_enters],
        ...
        for index, row in corporate_bonds.iterrows():
            cnpj1 = row[self.graph_node_matrix_1] if row[self.graph_node_matrix_1] < row[self.graph_node_matrix_2] else row[self.graph_node_matrix_2]
            cnpj2 = row[self.graph_node_matrix_2] if row[self.graph_node_matrix_1] < row[self.graph_node_matrix_2] else row[self.graph_node_matrix_1]
        ...
        for cnpj1, cnpj2, bidding in bonds_dict:
            ...
            self.dict_graphs[bidding].add_edge(int(cnpj1),int(cnpj2), weight= weight)
