...
do
	...
	grep -q ";.*\b$(printf $procurements | sed -e 's/#[0-9.]*//g' -e 's/,/\\b.*,.*\\b/g')\b" || maximal_cross_graph_2_quasi_clique
done
...
then
	...
	superset_of_procurements="\b.*;.*\b$(printf $procurements | sed -e 's/#[0-9.]*//g' -e 's/,/\\b.*,.*\\b/g')\b"
	grep -e "\b$cnpj1\b.*,$cnpj2$superset_of_procurements" -e "\b$cnpj2\b.*,$cnpj1$superset_of_procurements" $TMP/maximal-cross-graph-3+-quasi-cliques || maximal_cross_graph_2_quasi_clique
