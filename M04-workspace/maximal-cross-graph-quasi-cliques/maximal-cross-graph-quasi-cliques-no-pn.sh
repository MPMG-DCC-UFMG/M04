...
" > $TMP/edge
	...
	multidupehack --ids \; --ods \; -e "2147483648 2147483648 $epsilon" -c 0\ 1 -s 3 -g $TMP/edge |
	./canonical-output-no-pn.awk -F \;
}
...
BEGIN {
    if (threshold_for_2_cliques_for_2_cliques < threshold_for_3_cliques_or_more) {
        smaller_threshold = threshold_for_2_cliques_for_2_cliques
        ...
        greater_threshold = threshold_for_2_cliques_for_2_cliques
...
do
    ...
    procurements=$(grep -e "^$cnpj1;$cnpj2" -e "^$cnpj2;$cnpj1" "$1" | awk -F \; -v threshold=$min_weight_for_2_cliques '$4 >= threshold { print $3 }' | sort -n | tr \\n , | sed 's/,$//')
    maximal_cross_graph_3_quasi_cliques_or_more "$1" |
	tee -a $TMP/maximal-cross-graph-3+-quasi-cliques |
	grep -q ";.*\b$(printf $procurements | sed 's/,/\\b.*,.*\\b/g')\b" || printf "$cnpj1,$cnpj2;$procurements
"
done
...
then
	...
	superset_of_procurements="\b.*;.*\b$(printf $procurements | sed 's/,/\\b.*,.*\\b/g')\b"
	grep -e "\b$cnpj1\b.*,$cnpj2$superset_of_procurements" -e "\b$cnpj2\b.*,$cnpj1$superset_of_procurements" $TMP/maximal-cross-graph-3+-quasi-cliques || printf "$cnpj1,$cnpj2;$procurements
"
    done < $TMP/pairs_for_2-cliques
