// Copyright 2013-2020 Loïc Cerf (lcerf@dcc.ufmg.br)
...
/* MIN_AREA_REFINEMENT turns on the heuristic that refines the minimal area to remove more elements with MIN_SIZE_ELEMENT_PRUNING. */
#define MIN_AREA_REFINEMENT

/* NON_EXTENSION_ELEMENT_DETECTION turns out the heuristic that removes the elements that are set absent by enumeration and cannot prevent the closedness of the patterns that will be recursively considered. */
#define NON_EXTENSION_ELEMENT_DETECTION
...
/* TIME turns on the output (on the standard output) of the run time of multidupehack. */
/* #define TIME */
